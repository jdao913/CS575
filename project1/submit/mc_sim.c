...
int main(int argc, char *argv[ ]) {
    ...
    if (argc != 3) {
        fprintf(stderr, "Error: Requires 2 arguments of how many threads to use (1) and how many trials to run (2).\n");
        return 0;
    }
    ...
    printf("Running %i trials using %i thread(s)\n", NUMTRIALS, NUMT);
    ...
    for( int t = 0; t < NUMTRIES; t++ ) {
        ...
        int numHits = 0;
        #pragma omp parallel for default(none) shared(xcs,ycs,rs,tn, NUMTRIALS) reduction(+:numHits)
        for( int n = 0; n < NUMTRIALS; n++ ) {
        ...
        avgPerformance += (megaTrialsPerSecond - avgPerformance) / (t+1);
        if( megaTrialsPerSecond > maxPerformance )
            maxPerformance = megaTrialsPerSecond;
        currentProb = (float)numHits/(float)NUMTRIALS;
        char* outstring;
        sprintf(outstring, "Current Prob: %.3f\tMax Performance: %.3f\tAvg Performace: %.3f", currentProb, maxPerformance, avgPerformance);
        if (t == NUMTRIES-1) {
            strncat(outstring, "\n", 1);
        } else{
            strncat(outstring, "\r", 1);
        }
        // printf(outstring);
        // fflush(stdout);
    }
    printf("Prob: %.3f\tMax Performance: %.3f\tAvg Performace: %.3f\n", currentProb, maxPerformance, avgPerformance);
}