...
{
	if (argc != 3) {
		fprintf(stderr, "Error: Requires exactly two arguments of number of elements and local size.\n");
		return 1;
	}
	int NUM_ELEMENTS = std::stoi(argv[1]);
	int LOCAL_SIZE = std::stoi(argv[2]);
    ...
    // Check sum
    // float expected = .1*.1*NUM_ELEMENTS;
    // if( fabs( sum - expected ) > TOL )
    // {
    //     fprintf( stderr, "NUM_ELEMENTS %i wrongly produced %13.6f instead of %13.6f (%13.8f)\n",
    //         NUM_ELEMENTS, sum, expected, fabs(sum-expected) );
    // }
