...
__global__  void MonteCarlo( float *Xcs, float *Ycs, float *Rs, int *Hits, int *ids)
{
	...
	// printf("gid: %i\n", gid);
	// if (gid > NUMTRIALS) {
	// 	printf("gid greater than numtrials\n");
	// }
...
{
	TimeOfDaySeed( );
	FILE *fp = fopen("./mc_sim_data.csv", "w+");
	fprintf(fp, "Block Size, Number of Trials, Max Performance, Avg Performance\n");
	int dev = findCudaDevice(argc, (const char **)argv);

	int blocksize_arr[4] = {16, 32, 64, 128};
	int numtrials_arr[7] = {16*1024, 32*1024, 64*1024, 128*1024, 256*1024, 512*1024, 1024*1024};

	float maxPerformance = 0.;      // must be declared outside the NUMTRIES loop
    float avgPerformance = 0.;      // must be declared outside the NUMTRIES loop

	for (int b = 0; b < 4; b++) {
		int BLOCKSIZE = blocksize_arr[b];
		for (int t = 0; t < 7; t++) {
			int NUMTRIALS = numtrials_arr[t];
			...
			for (int j = 0; j < NUMTRIES; j++) {
				...
				status = cudaMalloc( (void **)(&dids), NUMTRIALS *sizeof(int) );
				checkCudaErrors( status );
				...
				MonteCarlo<<< grid, threads >>>( dXcs, dYcs, dRs, dHits, dids );
				...
				avgPerformance += megaTrialsPerSecond;
				// copy result from the device to the host:

				status = cudaMemcpy( hHits, dHits, NUMTRIALS *sizeof(int), cudaMemcpyDeviceToHost );
				status = cudaMemcpy( ids, dids, NUMTRIALS *sizeof(int), cudaMemcpyDeviceToHost );
				checkCudaErrors( status );
				cudaDeviceSynchronize( );
			...
			fprintf(fp, "%i, %i, %f, %f\n", BLOCKSIZE, NUMTRIALS,  maxPerformance, avgPerformance);
		}
	}
